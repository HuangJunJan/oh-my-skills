...
| Skill | 主题 | 何时生效 |
|---|---|---|
| `oms-meta` | 最高优先级元规则：优先级、事实核验、工具能力、平台基线、证据完整性、skill 维护边界 | 每轮对话首轮；任何代码改动前；任何 oms-* skill 调用前 |
| `oms-qa` | 通用问答交互：中文默认、先结论、少问多查、推荐优先、盘问 / 严审模式 | 每轮对话默认；用户要求盘问、严审、挑战方案、stress-test、grill me 时 |
