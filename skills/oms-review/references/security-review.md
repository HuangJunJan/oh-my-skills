...
`$oms-review` §3「安全」的展开。逐项排查注入、认证授权、敏感数据、常见漏洞;每条给 ❌/✅ 对照。命中后按 §2 定级(注入/越权/密钥泄露通常是 Blocker,缺校验/弱配置通常是 Major)。具体修复纪律见 `$oms-be-coding`。
...
```typescript
// ❌ 改状态的接口无 CSRF 防护、cookie 无 SameSite
